...
### 8.1 贴图缺口 (2026-08-12 后全部补齐)

| 项目 | 现状 | 证据 |
|---|---|---|
| 技能栏图标 | ✅ 12 技能专属图标已产出并接入 (替换手柄键帽) | hud.ts `SKILL_ICON_BY_ID`、icons.json `skill_*` |
| 药水图标 | ✅ potion_hp/potion_mp 已接入 HUD 与商店 | hud.ts、main.ts:1588 |
| 材料/道具图标 | ✅ mat_* (灵铁/奥术核心/虚空碎片) 已接入城镇面板 | main.ts:1594,1607 |
| 图标库 | ✅ 素材经 ai-gen 管线产出 → atlas 打包 | assets/atlas/input/icons/ |

### 8.3 特效缺失 (2026-08-12 后已补齐)

| 技能/机制 | 视觉 |
|---|---|
| 旋风斩/冰霜新星 | ✅ castAoe 扩散环 + 爆裂 (aoeVisual 表) |
| 闪电链 | ✅ spawnBolt 三段连线 + 端点爆点 |
| 终极 | ✅ 爆炸波 + 星屑 (spawnRing 200px + burst star_03) |
| 回血 heal | ✅ 绿色辉光 + 粒子 |
| 护盾/毒池/自爆 | ✅ 护盾青色脉冲弧 / 毒池呼吸圈 / 自爆红环+爆裂 |
| 新星/狂暴/召唤精英 | ✅ 对应环+爆裂 / 红环红光 / 召唤环 |
| 受击/翻滚/连击/掉金 | ✅ 红爆红环 / 起跳尘雾 / 里程碑环 / 金爆点 |
| 精英出生 / 二阶段 / 死亡触发 | ✅ 紫环 / 橙环爆裂 / 炸裂·分裂·毒池 |

### 8.2 特效现状 (已实现: 通用粒子 + additive 合批 + vfx.ts 发射器)

- vfx.ts: ring/burst/bolt/glow/impact 表驱动发射器 (生命周期/淡出 tint×fade)
- 接入: 技能 muzzle/AOE 环/闪电连线/治疗辉光/终极爆波、怪物全机制环爆、受击/翻滚/连击/掉金、死亡触发三型、撞墙火花

### 8.4 结论与优先级 (完成)

- particles.json 96 粒子中 flame*/light*/circle* 已大量启用 (仍有余量供后续内容)
- 全部 4 项已落地: ① VFX 发射器 ✅ ② 材料/药水图标 ✅ ③ 技能图标 ✅ ④ 怪物机制视觉 ✅
...
| B | 技能/药水/材料图标 + NPC 素材 + VFX 发射器全机制接线 + 护盾弧/毒池/燃烧/荆棘视觉 | ✅ 全部完成 |
